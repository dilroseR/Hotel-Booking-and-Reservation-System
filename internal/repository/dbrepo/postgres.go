...
// SearchAvailabilityByDatesByRoomID return true if availability exists for roomID , false if no availability exists
...
func (m *postgresDBRepo) SearchAvailabilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	...
	query := `select r.id, r.room_name 
				...
				where 
				r.id not in 
				(select rr.room_id from room_restrictions rr  where $1 < rr.end_date and
							$2  > rr.start_date)`
...
//GetOneReservationByID returns one reservation by ID
func (m *postgresDBRepo) GetOneReservationByID(id int) (models.Reservation, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
